...
- как лабораторные результаты и наблюдения превращаются во временные ряды;
...
Поля `red_flags` и `ai_summary` входят в стабильную форму ответа.
`DashboardService` заполняет `red_flags` результатом детерминированного
`src/red_flags`, а проверенный результат `SummaryService` приложение добавляет
в `ai_summary` без изменения контракта v1.
...
После построения `metrics` сервис вызывает
`evaluate_red_flags(record, metrics, as_of=as_of)`. Правила используют готовые
нормализованные ряды для калия, HbA1c, eGFR, ЛПНП и АД, а каноническую историю —
для документированных диагнозов и приёмов офтальмолога. Это сохраняет
разделение ответственности: backend композирует ответ, rules engine определяет
сигналы, frontend их только форматирует.

Шесть текущих правил, точные границы, медицинские основания и ограничения
описаны в [документации красных флагов](red_flags.md).
