...
`DashboardResponse` с методом, конкретными входами и `source_ids` исходных
наблюдений.
...
- Каждый корневой контракт имеет собственный `schema_version`: текущий
  `PatientRecord` — `1.0`, текущий `DashboardResponse` — `1.1`.
- Каталоги `patient/v1` и `dashboard/v1` обозначают major-линию. Совместимые
  minor-версии `1.x` остаются внутри неё.
