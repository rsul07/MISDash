...
## Источники входного JSON

Parser принимает путь и намеренно не знает, откуда появился файл. В Streamlit
`src/app/source.py` выбирает загруженный JSON либо результат
[`src/generator`](generator.md), после чего `src/app/data.py` передаёт оба
варианта через один временный файл в `parse_record()`. Поэтому синтетический
режим проверяет тот же публичный вход parser и не обходит нормализацию или
контракт `PatientRecord`.

Сырой формат встроенного generator не является новым контрактом parser:
конкретные aliases и грязные формы остаются деталями входного адаптера.

Для проверки всего преобразования на нескольких воспроизводимых вариациях
используется `src/quality`. Он сверяет количества, ссылки, `source.path`,
лабораторные комментарии, числовые значения и даты с исходным JSON. Команды,
инварианты и границы такой проверки описаны в
[контроле качества parser](parser_quality.md).
