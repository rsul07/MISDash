...
точки имеют отдельную метку. Plotly использует фиксированные цвета серий из
палитры приложения, белый фон, спокойную сетку, единый hover и короткий
transition. `theme=None` не позволяет теме Streamlit переопределять эти
настройки.
