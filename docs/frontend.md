...
`MISParser.parse()` и не меняет Streamlit: загруженный файл обрабатывается
...
удаляет его в `finally`. Валидированный канонический payload и
`DashboardResponse` кешируются по содержимому файла. Детерминированные
компоненты получают Pydantic-модель `DashboardResponse`, поэтому не зависят от
имён ключей в грязном JSON. Только orchestration-код ИИ-сводки дополнительно
получает `PatientRecord`, чтобы выбрать свободный текст.
